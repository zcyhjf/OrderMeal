$(function () {
    ...
    $(".add").click(function(){
        ...
        findPidByProductName(name);
        
        
        console.log("=4567865688765689           "+$('#sid').val());
  ...
  //减的效果
    $(".ms").click(function () {
        ...
        if(n>1){
            ...
            var danjia = $(this).parent().children().last().text();//获取单价
            var a = $("#totalpriceshow").html();//获取当前所选总价
            $("#totalpriceshow").html((a * 1 - danjia * 1).toFixed(2));//计算当前所选总价
...
*/      var danjia = $('.pce').text();//获取单价
    ...
    function add(data) {
        ...
        $(".list-content>ul").append( '<li class="food"><div><span class="accountName" data-name="'+data[0]+'" data-icon="'+data[5]+'">'+data[0]+'</span><span class="accountSize" data-size="'+data[1]+'" data-icon="'+data[5]+'">'+data[1]+'</span></div><div><span>￥</span><span class="accountPrice" data-price="'+data[2]+'" data-icon="'+data[5]+'">'+data[2]+'</span></div><div class="btn"><button class="ms2" style="display: inline-block;"><strong></strong></button> <i style="display: inline-block;" class="accountCount">'+data[3]+'</i><button class="ad2"> <strong></strong></button><i style="display: none;">'+data[4]+'</i></div></li>');
        ...
        $(document).on('click','.ms2',function(){
            var a = parseInt($(this).parent().children().last().text());
    ...
    $(".left-menu li").click(function(){
        ...
        /*该元素增加一个类li_add同时它的兄弟元素去掉li_add类*/
        var n = $(".left-menu li").index(this);/*n是第几个li*/
    ...
    $(".xhlbtn").click(function(){
    	var str = "";
    	$(".food").each(function(){
    			accountName = $(this).children().children('.accountName').text();
    			accountSize = $(this).children().children('.accountSize').text();
    			accountPrice = $(this).children().children('.accountPrice').text();
    			accountCount = $(this).children().children('.accountCount').text();
    			str += accountName+"-"+accountSize+"-"+accountCount+"-"+accountPrice+"-";
    	})
//    	console.log("名  "+accountName);
//		console.log("规格  "+accountSize);
//		console.log("价格  "+accountPrice);
//		console.log("数量  "+accountCount);
    	console.log(str);
    	window.location.href="toOrderDetail?sid="+$('#sid').val()+"&str="+str;
    	/*window.location.href="toOrderDetail?sid=1&str="+str;*/
    })
